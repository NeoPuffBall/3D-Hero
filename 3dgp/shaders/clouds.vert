...
uniform float t, vertPos, transparency, a;
...
float wave(float a, float x, float y, float t){
	t *= 0.05;
	return a* texture(noise1, aTexCoord + vec2(x + (t * n1Speed.x), y * t * n1Speed.y)).r * 
...
void main(void) 
{
	...
	vec4 pos = matrixProjection * matrixView * vec4((newVertex*vec3(100,3,80))+vec3(-30,vertPos,10), 1.0);
	...
	float NdotL = max(dot(newNormal, L), 0.0);
